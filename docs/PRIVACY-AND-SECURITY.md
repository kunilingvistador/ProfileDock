...
По запросу владельца сайта добавлена интеграция Google Analytics 4 с явным согласием посетителя. Она относится только к сайту: исполняемый код приложения для Mac не изменён. До согласия Google tag не загружается и события Google Analytics не отправляются. Без согласия сайт остаётся доступным. Выбор хранится в localStorage до 180 дней; его можно изменить кнопкой «Настройки аналитики».
...
Cookies этого счётчика имеют префикс `pd` и путь `/ProfileDock/`. При отзыве согласия выставляется запрет отправки, очищаются эти cookies и перезагружается документ, если Google tag уже был загружен. Уже отправленные данные этим действием не удаляются. Изменение выбора синхронизируется между вкладками через localStorage. GitHub Pages продолжает отдельно обрабатывать запросы хостинга. [Google Privacy Policy](https://policies.google.com/privacy), [Basic consent mode](https://developers.google.com/tag-platform/security/concepts/consent-mode).
