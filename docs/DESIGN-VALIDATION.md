...
## Follow-up: separate profile shortcuts in the Dock — 2026-09-08

The RU and EN hero now explicitly describe one Dock shortcut per Chrome profile, with the existing-window binding explained immediately below. The interactive illustration pairs three permanent shortcut labels with the same large, original SVG artwork in their windows. A decorative Chrome icon provides Dock context; the three profile buttons switch only the website demo. The actual app and release binaries are unchanged.

Validation: both static routes built successfully; SEO/export checks passed for both pages, the sitemap and 40 local HTML asset references. The three exported SVG files match their sources. Targeted lint and `git diff --check` passed. In-browser clicks through all three RU profiles kept the selected button, foreground window and live status in agreement; Enter switched the Project profile. EN navigation and Personal switching passed. At a 320 px viewport (305 px content excluding the scrollbar), neither locale overflowed horizontally, and all three 52 × 72 px profile buttons retained visible labels. Desktop and mobile screenshots were inspected; no console warnings or errors were recorded. This is website-only validation, not a new live Chrome or native app benchmark.
