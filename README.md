...
Switching does not use the network. The optional website-icon downloader makes web requests: the destination receives your IP address and requested URL, without Chrome cookies or profile data. Choose a local image file to avoid those requests. The project website and downloads are hosted by GitHub, whose service logging is separate from the app. The website also offers optional, consent-based Google Analytics for visits and download-link clicks; this is separate from the native app, which has no analytics. The published **0.1.5 beta** adds stricter icon downloads, private storage permissions and an in-app Data and permissions screen. See [the privacy review, data map, and remaining limits](docs/PRIVACY-AND-SECURITY.md).
